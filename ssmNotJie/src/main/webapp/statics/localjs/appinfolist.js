...
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			alert("该手游应用无版本信息，请先增加版本信息！");
		}else{
			window.location.href="appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
		alert("该手游应用的状态为：【"+obj.attr("statusname")+"】,不能修改其版本信息，只可进行【新增版本】操作！");
	}
});
$(".modifyAppInfo").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		...
		alert("该手游应用的状态为：【"+obj.attr("statusname")+"】,不能修改！");
	}
});
...
$(document).on("click",".saleSwichOpen,.saleSwichClose",function(){
	...
	if("open" === saleSwitch){
		...
		if(confirm("你确定要下架您的手游应用【"+obj.attr("appsoftwarename")+"】吗？")){
			saleSwitchAjax(appinfoid,obj);
		}
	}
});
		...
		success:function(data){
			 ...
			 */
				if(data.resultMsg === "success"){//操作成功
					...
					if("open" === obj.attr("saleSwitch")){
						alert("【"+obj.attr("appsoftwarename")+"】的【上架】操作失败,请联系管理员");
					}else if("close" === obj.attr("saleSwitch")){
						alert("【"+obj.attr("appsoftwarename")+"】的【下架】操作失败,请联系管理员");
					}
				}
		},
		error:function(data){
...
$(".deleteApp").on("click",function(){
	var obj = $(this);
	if(confirm("你确定要删除手游应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:"delapp.json",
			...
			success:function(data){
				if(data.delResult == "true"){//删除成功：移除删除行
					...
					alert("对不起，手游应用【"+obj.attr("appsoftwarename")+"】不存在");
				}else{
					   alert("系统繁忙，请稍后再试");
				}
			},
